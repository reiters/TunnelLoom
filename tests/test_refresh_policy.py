...
def test_main_window_has_no_periodic_refresh_timer():
    ...
    ).parents[1] / "tunnelloom_gui" / "main_window.py").read_text(encoding="utf-8")
...
def test_refresh_interval_setting_removed():
    ...
    assert "refresh_seconds" not in (
        root / "tunnelloom_gui" / "types.py").read_text(encoding="utf-8")
    assert "Refresh interval" not in (
        root / "tunnelloom_gui" / "dialogs.py").read_text(encoding="utf-8")
...
def test_refresh_uses_vpncmd_state_loader_without_periodic_polling():
    ...
    backend_source = (Path(__file__).resolve(
    ).parents[1] / "tunnelloom_gui" / "backend.py").read_text(encoding="utf-8")
    ...
    stable_block = backend_source.split("def load_stable_state_if_running", 1)[
        1].split("def wait_for_state", 1)[0]
