from tunnelloom_gui.backend import SoftEtherBackend, quote
from tunnelloom_gui.types import AccountProfile, AppConfig, CommandResult
...
def test_edit_does_not_overwrite_unknown_proxy_retry_or_startup_defaults():
    ...
    commands, _ = backend()._profile_commands(
        profile, creating=False, original_name="workaccount")
    joined = "\n".join(commands)
...
def test_client_status_recognizes_stopped_even_with_nonzero_return(monkeypatch) -> None:
    ...
    backend = SoftEtherBackend(AppConfig(
        softether_dir="/tmp/vpnclient", vpnclient_path="/tmp/vpnclient/vpnclient"))
    monkeypatch.setattr(
        backend,
        "_privileged_request",
        lambda *_args, **_kwargs: CommandResult(
            1, "SoftEther VPN Client service is not running.", ""),
    )
...
def test_load_state_if_running_treats_local_connection_failure_as_stopped(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        backend,
        "load_state",
        lambda: (_ for _ in ()).throw(VpncmdError(
            "Connection to server failed", failure)),
    )
...
def test_load_state_if_running_does_not_hide_other_vpncmd_errors(monkeypatch) -> None:
    ...
    monkeypatch.setattr(backend, "load_state",
...
def test_stable_refresh_waits_for_expected_visible_state_without_status_preflight(monkeypatch) -> None:
    ...
    calls = []
    monkeypatch.setattr(backend, "client_is_running", lambda: False)
    monkeypatch.setattr(backend, "load_state", lambda: calls.append(
        "AccountList/NicList") or expected)
...
def test_interface_ipv4_address_reads_first_global_address(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tunnelloom_gui.backend.shutil.which",
                        lambda name: "/usr/sbin/ip" if name == "ip" else None)
    monkeypatch.setattr("tunnelloom_gui.backend.subprocess.run", fake_run)
...
"""
    backend = SoftEtherBackend(AppConfig())
    monkeypatch.setattr(
        backend, "run", lambda _commands: CommandResult(0, output, ""))
    calls = []
    monkeypatch.setattr(
        backend,
